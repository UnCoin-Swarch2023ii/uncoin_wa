...
const Page = () => {
  ...
  const onFinishC = async (values: any) => {
    ...
    try {
      const resp = await comSignIn(values);
      console.log(resp);
      if (resp.status === 200) {
  ...
  return (
    <div className="containerLogin">
      <style>
        @import
        url('https://fonts.googleapis.com/css2?family=Lora&family=Montserrat&family=Playfair+Display:ital@0;1&family=Poppins&display=swap');
      </style>
          ...
          <Space direction="vertical" size="large">
            ...
            >
              <Form.Item
                name="nit"
                label="NIT"
                rules={[{ required: true, message: "Porfavor ingrese su nit" }]}
              >
                <InputNumber
                  maxLength={16}
                  minLength={8}
